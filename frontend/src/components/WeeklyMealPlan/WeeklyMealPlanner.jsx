...
const WeeklyMealPlanner = () => {
  ...
  const handleMeal = (day, type, action) => {
    if (action === 'add') {
      ...
      const suggestions = [
        'Avocado Toast', 'Pasta Bake', 'Grilled Chicken', 'Veggie Wrap', 'Salmon Bowl',
        'French Toast', 'Tomato Soup', 'Pita Pizza', 'Fruit Salad'
      ];
          ...
          {mealTypes.map((type) => (
            <div className="meal-row" key={type}>
              ...
              <div className="meal-actions">
                <button onClick={() => handleMeal(day, type, 'add')}>Add</button>
                <button onClick={() => handleMeal(day, type, 'suggest')}>Suggest</button>
              </div>
            </div>
          ))}
        </div>
